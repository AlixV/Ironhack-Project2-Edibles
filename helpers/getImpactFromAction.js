...
module.exports = function getImpactFromAction(action, plant) {
  // init
  const impact = { msg: "", classNameForMsg: "", lifeImpact: 0, isChoiceOk: false };
  ...
  if (plant.isEdible) {
    if (action === dataGame.ACTION_EAT) {
      ...
      impact.classNameForMsg = dataGame.MSG_SUCCESS_CSS_CLASSNAME;
    ...
    if (action === dataGame.ACTION_LEAVE) {
      ...
      impact.classNameForMsg = dataGame.MSG_FAILURE_CSS_CLASSNAME;
  ...
  if (plant.isLethal) {
    if (action === dataGame.ACTION_EAT) {
      ...
      impact.classNameForMsg = dataGame.MSG_OOPS_CSS_CLASSNAME;
